...
import 'package:accountmanager/app/web_view_home/overview/summary_table.dart';
import 'package:accountmanager/app/web_view_home/overview/tbr_questionnarie_start.dart';
...
class CreateOverviewSelectDataTableWidget extends ConsumerWidget {
  ...
  Widget build(BuildContext context, ScopedReader watch) {
    ...
    return DataTableBuilder(
        assignedDataAsync: assignedTbrAsyncValue,
        mobile: mobile,
        filterCompanyText: filterCompanyText,
        filterTechnicianIDText: technicianIDFilter,
...
class DataTableBuilder extends ConsumerWidget {
  ...
  final bool? mobile;
  final String? filterCompanyText;
  final String? filterTechnicianIDText;
  ...
  Widget build(BuildContext context, ScopedReader watch) {
    return assignedDataAsync!.when(
      data: (items) {
        final List<AssignedTBR> filteredCompanyItems = items.where((element) {
          if (filterCompanyText == null) {
            return true;
          } else {
            return element.company.name!
                .toLowerCase()
                .contains(filterCompanyText!.toLowerCase());
          }
        }).toList();

        final List<AssignedTBR> filteredTechnicianIDItems =
            filteredCompanyItems.where((element) {
          if (filterTechnicianIDText == null || filterTechnicianIDText == '') {
            return true;
          } else {
            return element.technicianIds.contains(filterTechnicianIDText);
          }
        }).toList();

        final List<AssignedTBR> filteredStatusItems =
            filteredTechnicianIDItems.where((element) {
          if (filterStatus == null) {
            return true;
          } else {
            return element.status.getStatusName() ==
                filterStatus!.getStatusName();
          }
        }).toList();

        return items.isNotEmpty
            ? ShowDataTable(items: filteredStatusItems, mobile: mobile)
            : const EmptyContent();
      },
...
Future<void> _displayNewTBREvalPage(
  ...
  Widget frame =
      Container(color: Colors.white, child: const TBRQuestionnaireStart());
  if (mobile) {
    // frame = Expanded(
