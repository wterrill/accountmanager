...
all.forEach(collectionName => {
  ...
  firestore_prod.collection(collectionName).get().then((res) => {
    // this cycles through the docuement array 
    res.docs.forEach(element => {
      // this takes the data from the _prod  and uploads it to the _dev using the collection name and the id of the document
      firestore_dev.collection(collectionName).doc(element.id).set(element.data()).then((res) => {
